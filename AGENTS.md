...
- Imported and refreshed data is untrusted (subscription servers and share links are attacker-controllable):
  ...
  - Subscription refreshes must not install routing rules from the response; rules only apply through reviewed manual imports. When a refresh removes an explicit local rule target, preserve the rule and route it to Reject rather than deleting it into a later fallback.
  - WireGuard refreshes must preserve peer keys, PSKs, endpoints, and allowed-IP routing until reviewed; compare effective peer values rather than generated peer IDs.
...
- `HopAppDataStore.save` stages immutable generation-scoped Keychain secrets, then atomically commits one envelope containing the authenticated redacted state, generation, and expected secret keys. Only after commit may it mirror stable tunnel-token accounts and prune obsolete generations. Loading must authenticate before decoding/hydrating and reject missing snapshot secrets, never fall back to mutable runtime aliases. Keep the signed legacy reader and recoverable unsigned-state authentication bootstrap for migration.
- `SecretWriteCache` reuses the committed generation and skips profile-Keychain writes when the secret set is unchanged; the first save after launch always writes. Any staging, file, alias, or prune failure invalidates the cache and returns failure for the UI to surface. Secret changes must flow through profile/subscription values, not side channels (legacy stable aliases remain compatibility-only; the extension never resolves credentials from them).
- `SecretStore.replaceAll` upserts new items before pruning stale keys and never prunes after a failed upsert. Preserve committed app-state generation keys passed by the caller. Never reintroduce a clear-then-rewrite pass: the tunnel extension can resolve secrets concurrently and must never observe an empty keychain.
...
- The shared App Group tunnel config is authenticated with `TunnelConfigAuthenticator` before the extension resolves nonce-bound secret tokens from it. Config/signature files and credentials are immutable per nonce; runtime credentials live under nonce-prefixed accounts in `SecretStore.runtime`, never mutable profile aliases. Retain installed/previous snapshots while pruning abandoned ones. Pre-snapshot configurations require manual reconnect. Keep the HMAC key in `SecretStore.runtime` and fail closed if the signature/key is missing or invalid.
