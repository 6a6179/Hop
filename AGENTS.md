...
- `HopAppDataStore.save` skips the Keychain `replaceAll` when the secret set is unchanged since the last write (`SecretWriteCache`); the first save after launch always writes, and a failed `replaceAll` (the `SecretBackend` write methods return success flags for this) invalidates the cache so the next save retries instead of skipping forever. Secret changes must flow through profile/subscription values so the cache observes them — do not write profile secrets or token-bearing subscription URLs to the Keychain through side channels the cache can't see (the connect-time re-ensure in `TunnelController` writes identical values and is fine).
